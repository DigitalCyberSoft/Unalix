...
def test_unshort():
    ...
    assert unalix.unshort_url(unmodified_url, allow_private=True) == f"{base_url}/ok"
    assert event_loop.run_until_complete(unalix.aunshort_url(unmodified_url, allow_private=True)) == f"{base_url}/ok"
